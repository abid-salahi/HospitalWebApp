    ...
    <body>
        ...
        <div class="card-deck">
            <div class="card border-dark" id = "appt-card">
                ...
                <div class="card-body">
                    <h5 class="card-title">All Doctors who have had sessions with all appointments</h5>

                    <?php
                    $techAppt = "SELECT * FROM `Doctor` d, `Staff` s WHERE d.sID = s.sID AND NOT EXISTS (SELECT `aID` FROM `Appointment` a1 WHERE `aID` NOT IN (SELECT `aID` FROM `Appointment` a2 WHERE a2.sID=d.sID))";
                    if($result = mysqli_query($link, $techAppt)){

                        while($row = mysqli_fetch_array($result)){
                            echo "<p>Name: ".$row['name']."</p>
                                      <p>Specification: ".$row['dSpec']."</p>
                                      <p>Education: ".$row['Edu']."</p><hr>";
                        }
                    }else{
            ...
            <div class="card border-dark">
                ...
                <div class="card-body">
                    ...
                    if ($res_max = mysqli_query($link, $max_count_appt)){
                        ...
                        <p>staff id: ". $row['sID'] . ", Max Count: " . $row['maxApptCount'] . "</p>";
                    } else {
